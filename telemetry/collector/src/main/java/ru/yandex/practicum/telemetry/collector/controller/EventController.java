...
/**
 * {@link EventController} — это {@code gRPC}-сервис для обработки событий датчиков и хаба, получаемых от {@code Hub Router}.
 * <p>
 * Он реализует gRPC API, определённый в {@link CollectorControllerGrpc.CollectorControllerImplBase},
 * предоставляя два RPC-метода: {@code collectSensorEvent} и {@code collectHubEvent}.
 * <p>
 * Этот сервис отвечает за:
 * <ul>
 *   <li>Приём событий датчиков через метод {@code collectSensorEvent}.</li>
 *   <li>Приём событий хаба через метод {@code collectHubEvent}.</li>
 *   <li>Направление событий к соответствующему обработчику на основе типа данных события.</li>
 *   <li>Обработку ошибок и возврат соответствующих ответов gRPC-клиенту.</li>
 * </ul>
 */
