...
/**
 * Пользовательская реализация {@link ErrorDecoder} для Feign, предназначенная для обработки ошибок от Feign-клиентов. Декодирует
 * полученный ответ от Feign в исключение, специфичное для приложения, обеспечивая осмысленную передачу исключений
 * от удаленных сервисов. Использует стандартный декодер Feign для возвращения общего исключения по умолчанию.
 */
